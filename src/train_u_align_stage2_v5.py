"""
Stage 2 v5: v4 recipe (EOS targets) from scratch, 2 epochs. Isolated output paths.
...
OUTPUT_DIR = '/data/workspace/asr-model-training/thai-understanding/u_align_stage2_v5'
RESUME_FILE = '/data/workspace/asr-model-training/thai-understanding/u_align_stage2_v5_resume.json'
EVAL_HISTORY = '/data/workspace/asr-model-training/thai-understanding/u_align_stage2_v5/eval_history.jsonl'
...
def collate_batch(batch, encoder, adapter, tokenizer, base_model):
    ...
    for b in batch:
        ...
        T = se.shape[1]
        speech_embeds_list.append(se.squeeze(0))
    ...
    # CTC side: per-sample speech frames (pre-concat) + unit-id targets
    max_speech = max(spe.shape[0] for spe in speech_embeds_list)
    speech_padded = torch.zeros(B, max_speech, 2560, dtype=torch.bfloat16, device='cuda')
    for i, spe in enumerate(speech_embeds_list):
        speech_padded[i, :spe.shape[0]] = spe
    ctc_target_lens = torch.tensor([len(b.get('ctc_ids', [])) for b in batch], dtype=torch.long)
    ctc_targets = torch.tensor([t for b in batch for t in b.get('ctc_ids', [])], dtype=torch.long)

    return {
        'inputs_embeds': final_embeds,
        'labels': final_labels,
        'attention_mask': final_mask,
        'speech_embeds': speech_padded,
        'speech_lens': torch.tensor([spe.shape[0] for spe in speech_embeds_list], dtype=torch.long),
...
# ============ Training & Eval ============
def train(args):
    print("=== Stage 2 v5: REPLACE + EOS targets + CTC aux loss ===\n")
    ...
    print(f"\n[1b/6] Building CTC unit vocab (Mn marks merged)...")
    vocab, unit_cnt = build_vocab([s['ref'] for s in train_data], min_freq=5)
    inv_vocab = {v: k for k, v in vocab.items()}
    for s_ in train_data:
        s_['ctc_ids'] = text_to_ids(s_['ref'], vocab)
    Path(OUTPUT_DIR, 'ctc_vocab.json').write_text(json.dumps(vocab, ensure_ascii=False, indent=1))
    print(f"  vocab={len(vocab)} units (blank+space incl); targets use frequent units only")
    ...
    print(f"\n[3b/6] CTC head: Linear(2560 -> {len(vocab)}) on adapter frames (fp32)...")
    ...
    print(f"\n[5/6] Loading F2LLM-v2-4B (CPU; NOT used during training — GPU shared, deferred to final eval)")
    # Do NOT load F2LLM during training: GPU is shared with the align service;
    # quick-eval metrics = CTC-CER + diversity. F2LLM final eval runs post-training.
    embed_model = None
    ...
    def run_eval_and_log(step, data, subset_name):
        ser, sims, refs, hyps, ctc_cer_ns = quick_eval(
            adapter, tokenizer, base_model, encoder, embed_model, data,
            ctc_head=ctc_head, inv_vocab=inv_vocab)
        ...
        ser_s = f"{ser:.4f}" if ser is not None else "n/a (F2LLM deferred)"
        print(f"  >> step {step} {subset_name} SER = {ser_s}  "
              f"CTC-CER(ns) = {ctc_cer_ns if ctc_cer_ns is None else round(ctc_cer_ns, 4)}  "
        ...
        with open(EVAL_HISTORY, 'a') as f:
            f.write(json.dumps({'step': step, 'subset': subset_name, 'ser': ser,
                                ...
                                'ctc_cer_ns': ctc_cer_ns,
                                'sims': sims, 'hyps': hyps, 'refs': refs}) + '\n')
        return ser, hyps, ctc_cer_ns
    ...
    optimizer.zero_grad()
    t_start = time.time()
    best_sel = resume.get('ctc_cer_ns', 1.0) if resume else 1.0  # selection on CTC-CER
    ...
    for epoch in range(args.epochs):
        for batch_idx, batch in enumerate(train_loader):
            ...
            log_probs = F.log_softmax(ctc_head(batch['speech_embeds'].float()), dim=-1).transpose(0, 1)  # (T,B,V)
            ctc_loss = F.ctc_loss(
                log_probs, batch['ctc_targets'],
                batch['speech_lens'].clamp(min=1), batch['ctc_target_lens'],
                blank=0, zero_infinity=True)
            loss = (ce_loss + args.ctc_weight * ctc_loss) / args.grad_accum
            loss.backward()
            accumulated_loss += loss.item()
            accumulated_ctc += ctc_loss.item()
            ...
            if (batch_idx + 1) % args.grad_accum == 0:
                ...
                if global_step % 20 == 0:
                    ...
                    print(f"  step {global_step}/{total_steps} loss={accumulated_loss:.4f} ctc={accumulated_ctc:.3f} {elapsed:.0f}s ETA {eta:.0f}s", flush=True)
                accumulated_loss = 0.0
                accumulated_ctc = 0.0
                ...
                if smoke_stop or global_step >= next_eval:
                    next_eval += args.eval_every
                    ser, hyps, ctc_cer = run_eval_and_log(global_step, eval_data_h173, 'h173')
                    # v5 monitoring metric = CTC-CER (content signal, LLM-independent);
                    # F2LLM SER deferred to post-training GPU window
                    sel = ctc_cer if ctc_cer is not None else 1.0
                    Path(RESUME_FILE).write_text(json.dumps({'step': global_step, 'ser': ser,
                                                             'ctc_cer_ns': ctc_cer}))
                    if sel < best_sel:
                        best_sel = sel
                        save_checkpoint(model, adapter, tokenizer, global_step, sel, ctc_head)
                        print(f"  >> step {global_step} saved (best CTC-CER={sel:.4f})")
                    if ctc_cer is not None and ctc_cer < args.target_ser:
                        print(f"  >> step {global_step} reached target CTC-CER < {args.target_ser}, stopping")
                        return
                    if smoke_stop:
    ...
    save_checkpoint(model, adapter, tokenizer, global_step, best_sel, ctc_head)
    # Final: full val pool silver subset (subset-consistency check vs h173, §4.6 #8)
    ser_final_silver, _, ctc_final_silver = run_eval_and_log(global_step, eval_data_silver, 'silver1284')
    print(f"\n=== Training done. Final step {global_step}. Best h173 CTC-CER={best_sel:.4f}, "
          f"final silver CTC-CER={ctc_final_silver} ===")


def quick_eval(adapter, tokenizer, base_model, encoder, embed_model, eval_data,
               ctc_head=None, inv_vocab=None):
    """REPLACE mode inference: [speech | prompt] → generate. F2LLM on GPU.
    Also runs CTC greedy decode on adapter frames as an LLM-independent
    content metric (CER no-space vs refs)."""
    adapter.eval()
        ...
        try:
            ...
            with torch.no_grad():
                ...
                if ctc_head is not None:
                    ids_seq = ctc_head(se.float().squeeze(0)).argmax(-1).cpu().tolist()
                    prev, out_ids = -1, []
                    for i in ids_seq:
                        if i != prev and i != 0:
                            out_ids.append(i)
                        prev = i
                    ctc_hyps.append(ids_to_text(out_ids, inv_vocab))
    ...
    ctc_cer_ns = None
    if ctc_head is not None and ctc_hyps:
        from eval_cer_pretrained import cer_no_space
        ctc_cer_ns = float(sum(cer_no_space(r, h) for r, h in zip(refs, ctc_hyps)) / max(1, len(ctc_hyps)))
    return ser, sims, refs, hyps, ctc_cer_ns


def save_checkpoint(model, adapter, tokenizer, step, ser, ctc_head=None):
    ...
    if ctc_head is not None:
        torch.save({'ctc_head_state_dict': ctc_head.state_dict()},
                   f'{OUTPUT_DIR}/checkpoints/step_{step}/ctc_head.pt')
...
if __name__ == '__main__':
    ...
    ap.add_argument('--ctc_weight', type=float, default=0.3, help='lambda for CTC aux loss')
    args = ap.parse_args()
    train(args)
