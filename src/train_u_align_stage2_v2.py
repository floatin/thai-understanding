"""
Stage 2 v2: REPLACE mode (per paper U-Align design).

Sequence structure (replace mode):
  [speech_embeds (T')] + [text_prompt_embeds (P)] + [text_answer_embeds (A)]

Compared to v1 (concatenation):
  v1: [prompt] + [speech] + [answer]   (speech between prompt and answer)
  v2: [speech] + [prompt] + [answer]   (speech BEFORE prompt, replaces first T' positions)

Reference: u-align-demo.py line 142
  inputs_embeds = self.llm.get_input_embeddings()(input_ids)
  inputs_embeds[:, :adapted_feats.shape[1], :] = adapted_feats

Key fixes from paper demo:
1. Speech REPLACES first T' positions of input_ids embeddings (not concatenate)
2. Labels correctly mask: positions [0, T'+P) = -100, positions [T'+P, T'+P+A) = answer_ids
...
Train on:  5159 PTT silver (excluding h173)
Quick eval: h173 with bge-m3-Thai cosine
Target:    SER < 0.10 on h173
"""
import warnings
...
OUTPUT_DIR = '/data/workspace/asr-model-training/thai-understanding/u_align_stage2_v2'
RESUME_FILE = '/data/workspace/asr-model-training/thai-understanding/u_align_stage2_v2_resume.json'

# Prompt - business context + hotwords + correction rules (NO [เสียง] marker - speech is already at front)
...
def collate_batch(batch, encoder, adapter, tokenizer, base_model):
    ...
    for b in batch:
        ...
        # Final: [speech (T)] + [text_prompt (P)] + [text_answer (A)]
        # This is achieved by CONCATENATING speech (which becomes the first T positions)
        # because the text embeddings will be at positions [T, T+P+A)
...
# ============ Training & Eval ============
def train(args):
    print("=== Stage 2 v2: REPLACE mode (paper design) ===\n")
    ...
    print(f"\n[5/6] Loading bge-m3-Thai judge (CPU)...")
...
def quick_eval(adapter, tokenizer, base_model, encoder, embed_model, eval_data):
    """REPLACE mode inference: [speech | prompt] → generate."""
    adapter.eval()
