"""
Compare trained vs random adapter on retrieval task.
"""
import warnings
...
# Load encoder + LLM embed_tokens (same as eval)
print("Loading encoder + LLM embed_tokens...")
...
# Two adapters: trained vs random
print("\nLoading trained adapter...")
ckpt = torch.load('/data/workspace/asr-model-training/thai-understanding/u_align_adapter/adapter_final.pt', map_location='cpu', weights_only=False)
trained_adapter = UAlignAdapter(encoder_dim=1024, llm_dim=2560, downsample=2).cuda().to(torch.bfloat16)
trained_adapter.load_state_dict(ckpt['adapter_state_dict'])
trained_adapter.eval()

print("Creating random adapter...")
torch.manual_seed(0)
...
df = pq.read_table('/data/workspace/asr-model-training/thai-understanding/Thai-SUP/SR/test/test-00000.parquet').to_pandas().head(64)
...
print("\nComputing trained adapter embeddings...")
a_trained, t = compute_embeddings(trained_adapter)
...
def retrieval_metrics(audio, text):
    ...
    metrics['off_diag'] = float(off_diag)
...
import json
out = Path('/data/workspace/asr-model-training/thai-understanding/phase1_adapter_compare.json')
out.write_text(json.dumps({'trained': m_trained, 'random': m_random}, indent=2))
print(f"\nSaved: {out}")
