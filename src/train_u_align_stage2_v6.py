...
OUTPUT_DIR = '/data/workspace/asr-model-training/thai-understanding/u_align_stage2_v6'
RESUME_FILE = '/data/workspace/asr-model-training/thai-understanding/u_align_stage2_v6_resume.json'
EVAL_HISTORY = '/data/workspace/asr-model-training/thai-understanding/u_align_stage2_v6/eval_history.jsonl'
...
# ============ Training & Eval ============
def train(args):
    print("=== Stage 2 v6: frozen CTC front-end + LLM LoRA (CE only) ===\n")
    ...
    print(f"\n[3/6] Loading CTC-trained adapter (FROZEN)...")
    ...
    for p in adapter.parameters():
        p.requires_grad = False
    adapter.eval()
    print(f"  (ctc_only epoch {ckpt.get('epoch')}, its h173 CER={ckpt.get('cer'):.4f})")

    print(f"\n[3b/6] CTC head (FROZEN diagnostic decoder, Linear(2560 -> {len(vocab)}))...")
...
def quick_eval(adapter, tokenizer, base_model, encoder, embed_model, eval_data,
    ...
    if embed_model is not None:
        ...
        torch.cuda.empty_cache()
    base_model.config.use_cache = False
    # adapter/ctc_head stay FROZEN in eval mode (v6)
...
def save_checkpoint(model, adapter, tokenizer, step, ser, ctc_head=None):
    ...
    torch.save({
        'adapter_source': CTC_ONLY_BEST,  # frozen front-end — not duplicated
        'encoder_dim': 1024, 'llm_dim': 2560, 'downsample': 2,
        'step': step, 'ser': ser,
    }, f'{OUTPUT_DIR}/checkpoints/step_{step}/adapter_ref.pt')
